...
void EyeBlink::Update() {
  ...
  if (Animation.GetElapsed() > Animation.Interval) t = 0.0f;
  Apply(t * t);
}
...
void EyeBlink::Apply(float t) {
  ...
  Output.Inverse_Radius_Top   = Input->Inverse_Radius_Top   * (1.0f - t);
  Output.Inverse_Radius_Bottom = Input->Inverse_Radius_Bottom * (1.0f - t);
  Output.Inverse_Offset_Top   = Input->Inverse_Offset_Top   * (1.0f - t);
  Output.Inverse_Offset_Bottom = Input->Inverse_Offset_Bottom * (1.0f - t);
}
